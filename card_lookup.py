...
def process_exact_match(group, cur):
    exact_sql = 'SELECT * FROM cards WHERE card_name = ? COLLATE NOCASE'
    return cur.execute(exact_sql, [group]).fetchone()


def process_card_id_match(group, cur):
    id_sql = 'SELECT * FROM cards WHERE card_id = ?'
    return cur.execute(id_sql, [group]).fetchone()
...
def generate_reply_message(results, cur):
    reply_message = ""
    if results:
        card_ids = []
        for r in results:
